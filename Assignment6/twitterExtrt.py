#Portions of this code was aquired from a colleague: Kevin Clemmons
#This particular script extracts the followers and the followers they are following

#My Twitter account did not have the minimum number of Followers, so Mr. Clemmons was kind of to use his account for data.
		...
		for follower in followerList:
			...
			if follower != user:
				...
				try:
					...
					printEdgeList(edgeList,'test.csv')
...
def retrieveFollowers(credentials,userName=None):
	...
	return followerList
#Prints the List of Followers from Kevins Twitter
...
if __name__=='__main__':
	configFileName = 'data.ini'
	credentialSet = credentials.twitter_credentials.TwitterCredentials(configFileName)
#Extracting Twitter data from Kevin Clemmon's Twitter account.
	followers = retrieveFollowers(credentialSet,userName='KevinClemmons')
